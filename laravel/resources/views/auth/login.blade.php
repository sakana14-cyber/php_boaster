<x-guest-layout>
    <h1 class="text-2xl text-black">ログイン</h1>

    <div class="w-full max-w-xs flex flex-col gap-12">
        <x-auth-session-status class="text-sm text-gray-600" :status="session('status')" />

        <form method="POST" action="{{ route('login') }}" class="flex flex-col gap-12">
            @csrf
            ...
            <div class="flex flex-col items-center gap-3">
                <button type="submit"
                        class="w-full h-10 px-3 bg-[#FF7E5F] text-white text-sm rounded shadow-[2px_4px_4px_rgba(139,152,220,0.25)]">
                    ログイン
                </button>
                ...
                <a href="{{ route('register') }}"
                   class="w-full h-10 px-3 flex items-center justify-center bg-white border border-[#FF7E5F] text-[#FF7E5F] text-sm rounded shadow-[2px_4px_4px_rgba(139,152,220,0.25)]">
                    新規登録
                </a>
            </div>
        </form>
